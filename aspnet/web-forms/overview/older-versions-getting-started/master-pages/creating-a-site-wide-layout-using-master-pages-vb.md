...
La première ligne dans le balisage déclaratif est la [ `@Master` directive](https://msdn.microsoft.com/library/ms228176.aspx). Le `@Master` la directive est similaire à la [ `@Page` directive](https://msdn.microsoft.com/library/ydy4x04a.aspx) qui s’affiche dans les pages ASP.NET. Il définit la langue de côté serveur (VB) et le plus d’informations sur l’emplacement et l’héritage de classe code-behind de la page maître.
