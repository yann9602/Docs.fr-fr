...
Dans ce didacticiel, vous allez déployer une application SignalR pour un rôle Web Windows Azure à l’aide de l’infrastructure d’intégration Service Bus pour distribuer les messages à chaque instance de rôle.
...
Dans le **nouveau Windows Azure Cloud Service** boîte de dialogue, sélectionnez rôle Web d’ASP.NET MVC 4. Cliquez sur le bouton de flèche droite (**&gt;**) pour ajouter le rôle à votre solution.
...
Dans le **nouveau projet ASP.NET MVC 4** Assistant, sélectionnez **Application Internet**. Cliquez sur **OK**. L’Assistant projet crée deux projets :
...
Pour créer l’application de conversation, suivez les étapes du didacticiel [prise en main SignalR et MVC 4](tutorial-getting-started-with-signalr-and-mvc-4.md).
