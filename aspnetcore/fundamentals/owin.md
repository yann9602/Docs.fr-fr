...
Par [Steve Smith](http://ardalis.com) et [Rick Anderson](https://twitter.com/RickAndMSFT)

ASP.NET Core prend en charge l’Interface Web ouverte pour .NET (OWIN). OWIN permet des applications web à être dissocié de serveurs web. Il définit une manière standard pour un intergiciel (middleware) à utiliser dans un pipeline pour gérer les demandes et réponses associées. Intergiciel (middleware) et les applications ASP.NET Core peuvent interagir avec intergiciel (middleware), les serveurs et les applications basées sur OWIN.
...
Remarque : À l’aide de ces adaptateurs assortie d’une baisse des performances. Les applications utilisant uniquement des composants ASP.NET Core ne devraient pas utiliser l’ou les adaptateurs Owin.

[Afficher ou télécharger l’exemple de code](https://github.com/aspnet/Docs/tree/master/aspnetcore/fundamentals/owin/sample)
...
| Touche               | Valeur (type) | Description |
| ----------------- | ------------ | ----------- |
| owin. ID de requête | `String` | Facultatif |
...
| serveur. RemoteIpAddress  | `String` | |    
| serveur. Port_distant | `String` | |     
| serveur. LocalIpAddress  | `String` | |    
| serveur. LocalPort  | `String` | |    
| serveur. IsLocal  | `bool` | |    
| serveur. OnSendingHeaders  | `Action<Action<object>,object>` | |
...
| opaque. Mise à niveau | `OpaqueUpgrade` | Consultez [signature du délégué](http://owin.org/spec/extensions/owin-SendFile-Extension-v0.3.0.htm) |
| opaque. Flux de données | `Stream` |  |
...
| WebSocket. Accepter | `WebSocketAccept` | Consultez [signature du délégué](http://owin.org/spec/extensions/owin-SendFile-Extension-v0.3.0.htm) |
| WebSocket. AcceptAlt |  | Non-spec |
| WebSocket. Sous-protocole | `String` | Consultez [RFC6455 Section 4.2.2](https://tools.ietf.org/html/rfc6455#section-4.2.2) étape 5.5 |
